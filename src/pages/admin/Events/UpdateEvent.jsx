...
const UpdatEvent = () => {
  const { loading, isAuthenticated, error, message, events } = useSelector(
    (state) => state.event
  );
  const params = useParams();
  let selectedItem = events.find((f) => f._id.toString() === params.id);
  ...
  useEffect(() => {
    if (!events.length) {
      ...
      const selectedItem = events.find((f) => f._id.toString() === params.id);
      if (selectedItem) {
  ...
  const submitHandler = (e) => {
    ...
    dispatch(updateEvent(myForm, params.id));
  };
